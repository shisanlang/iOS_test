...
//
//	Revision History
//	2011-06-21 | Charles | Full Screen Article Image Display.
...
- (void)viewDidLoad {
    ...
    CustomBarButtonItem *backButton = [[CustomBarButtonItem alloc] initWithTitle:@" " style:LeftBarButtonItemBackStyle target:self action:@selector(popViewController:)];
...
//    pagingScrollViewFrame.origin.y= -44;
...
//    UIButton *saveItem = [UIButton buttonWithType:UIButtonTypeCustom];
//    saveItem.frame = CGRectMake(0.0f, 8.0f, 54, 44);
//    [saveItem setTitle:@"保存" forState:UIControlStateNormal];
//    [saveItem addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
//    [saveItem setBackgroundImage:[UIImage imageNamed:@"downImage"] forState:UIControlStateNormal];
//    self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithCustomView:saveItem] autorelease];
...
//    self.view.backgroundColor = [UIColor colorWithRed:244.0/255.0 green:244.0/255.0 blue:244.0/255.0 alpha:1.0];
    ...
    if (photos.count > 1) {
        ...
        // Toolbar Items
        previousButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"left_able.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoPreviousPage)];
        nextButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"right_able.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
    ...
    downBut.frame = CGRectMake(XD_VIEWWIDTH-50-20, XD_SCREENHEIGHT-25-50, 50, 25);
    [downBut setImage:[UIImage imageNamed:@"download.png"] forState:UIControlStateNormal];
    [downBut setImage:[UIImage imageNamed:@"download_press.png"] forState:UIControlStateHighlighted];
    [downBut addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:downBut];
...
- (void)updateNavigation {
	...
	// Title
	if (photos.count > 1) {
		self.title = [NSString stringWithFormat:@"第%i/%i张", currentPageIndex+1, photos.count];		
	} else {
		self.title = nil;
	}
